...
export async function addAudioToMp4(
  ...
  for (const item of clips) {
    ...
    try {
      ...
      return fail(SystemFailureTag.FfmpegFailure, `Clip file not found: ${path}`);
    }
  ...
  try {
    ...
    return fail(SystemFailureTag.FfmpegFailure, `MP4 file not found: ${mp4Path}`);
  }
  ...
  if (audioSources.length === 1) {
    ...
    filterParts.push(`${audioSources.join("")}amix=inputs=${inputsCount}:duration=longest[aout]`);
  ...
  try {
    ...
    if (code !== 0) {
      ...
      return fail(SystemFailureTag.FfmpegFailure, `ffmpeg exited with code ${code}`);
    }
    ...
    try {
      ...
      return fail(SystemFailureTag.FfmpegFailure, `Failed to replace MP4: ${String(err)}`);
    }
  } catch (err) {
    return fail(SystemFailureTag.FfmpegFailure, `Unexpected error: ${String(err)}`);
  }
